...
This fork tracks upstream Komodo v2 and keeps extra agentic commands that are not in upstream:
...
This fork also reshapes the terminal UX from upstream v2:

- `km ssh host ...` opens an interactive host session
- `km ssh container ...` opens an interactive container session
- `km exec host ... -x '...'` runs a one-shot host command
- `km exec container ... -x '...'` runs a one-shot container command
- `km attach ...` attaches to a container's main process

Under the hood, these commands use the upstream v2 terminal target + init API.
...
Use `ssh` for interactive sessions:

```sh
km ssh host my-server
km ssh host my-server -t maintenance
km ssh container nginx --server my-server
km ssh container redis --server my-server -s bash
```

Use `exec` for one-shot commands:

```sh
km exec host my-server -x 'uname -a'
km exec host my-server -t maintenance -x 'df -h'
km exec container nginx --server my-server -x 'ls -la /'
km exec container redis --server my-server -s bash -x 'printenv'
```

Use `attach` only for container attach sessions:

```sh
km attach nginx --server my-server
km attach redis --server my-server -t attach-debug
```
