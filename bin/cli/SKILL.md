---
name: komodo-cli
description: Use when the user asks about managing their homelab, deployments, services, containers, stacks, servers, logs, builds, variables, procedures, syncs, or anything related to the Komodo infrastructure management CLI (`km`). Triggers on keywords like deploy, restart, logs, status, container, stack, server, build, variable, procedure, sync, homelab, infrastructure.
---
...
This is an **agentic fork** of upstream Komodo. It tracks upstream behavior for terminal/exec/attach and adds agent-friendly read commands: `stack`, `procedure`, `sync`, and `variable`.

## CRITICAL: SSH and remote commands

> **Do NOT use `km` to run shell commands on servers or inside containers.**

The CLI's terminal commands (`connect`, `exec`, `attach`) open **interactive, human-oriented**
WebSocket terminal sessions. They are not suitable for running one-shot or scripted commands,
and they are not suitable for agents.

To run commands on a host or inside a container:

1. **SSH directly into the server** with your SSH client: `ssh user@server`.
2. Run host commands directly, or run commands inside containers with
   `docker exec <container> <cmd>` / `docker compose exec <service> <cmd>`.

Keep `km` for the Komodo control plane (deploys, status, logs, variables, syncs, procedures).
Use real SSH for everything that is just "run this command over there".
...
### Interactive terminals (humans only)

These open live, interactive WebSocket sessions. They match upstream Komodo behavior and
are intended for a human at a keyboard, not for agents or scripted one-shot commands.

```sh
km connect my-server                 # Interactive server shell (alias: ssh)
km connect my-server bash            # ...with a custom shell
km exec nginx --server my-server bash   # Interactive shell in a container
km attach nginx --server my-server      # Attach to a container's main process
```

> To run commands inside containers or on servers programmatically, **SSH directly into the
> server** and use `docker exec` / `docker compose exec` from there. Do not use `km` for that.
...
- **Never suggest `km exec`/`km connect`/`km attach` for running commands programmatically.** For "run command X on server Y", tell the user to SSH directly into the server and run it there (use `docker exec` inside that SSH session for container commands).
